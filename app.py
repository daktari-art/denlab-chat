...
DEVELOPER_USERNAME = "dennis"  # Your username - change if different
...
def init_session():
    defaults = {
        ...
        "cache_enabled": True,
        "memory_enabled": True,
        "agent_max_steps": 25,  # Increased from 8 to allow complex tasks
        "is_developer": False,
    }
...
# ============ IMPROVED AGENT EXECUTION WITH HIGHER STEP LIMIT ============
...
def run_agent_task(prompt: str, model: str, progress_placeholder, user_id: str = None) -> Dict[str, Any]:
    ...
    traces = []
    # Use configurable max steps - increased for complex tasks
...
- Use write_file to save code and results
...
When you have the final answer, respond with just the answer (no tool calls)."""},
    ...
    # If we hit max steps, return partial progress
...
def show_user_menu():
    ...
    with st.sidebar:
        ...
        if st.session_state.is_developer:
            ...
            <div style="display:flex;align-items:center;gap:10px;padding:10px;background:#1a1a1a;border-radius:10px;margin-bottom:10px;border:1px solid #10a37f;">
                ...
                <div>
                    ...
                    <div style="font-size:11px;color:#10a37f;">👑 Developer</div>
                </div>
            </div>
            """, unsafe_allow_html=True)
        else:
        ...
        with st.expander("⚙️ Advanced Settings"):
            ...
            # Agent max steps slider - important for complex tasks
            st.session_state.agent_max_steps = st.slider(
                "Agent Max Steps", 
                min_value=5, 
                max_value=50, 
                value=st.session_state.get("agent_max_steps", 25),
                help="Increase for complex multi-step tasks like code generation or research"
            )
...
def show_settings():
    ...
    with tab1:
        ...
        if st.session_state.is_developer:
            st.info("👑 **Developer Mode Active** - You have access to advanced features.")
    ...
    with tab2:
        ...
        if st.button("Save Settings", type="primary"):
            auth.update_settings(st.session_state.user_token, {
                ...
                "agent_max_steps": st.session_state.agent_max_steps,
            })
...
def show_sidebar():
    ...
    with st.sidebar:
        ...
        if st.session_state.agent_mode:
            st.caption(f"🤖 Agent active • {st.session_state.agent_max_steps} steps max")
        ...
        st.divider()
        st.caption("🧠 Memory • ⚡ Cache • 🐙 GitHub • 👑 Dev Mode")
